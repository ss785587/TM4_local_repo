	...
	public function actionRecovery () {
		...
		if (Yii::app()->user->id) {
				...
				if ($email&&$activationKey) {
		    		...
		    		if(isset($find)&&$find->activationKey==$activationKey) {
			    	...
			    	if(isset($_POST['UserRecoveryForm'])) {
			    		...
			    		if($form->validate()) {
			    			$user = User::model()->notsafe()->findbyPk($form->idUser);
							$activation_url = 'http://' . $_SERVER['HTTP_HOST'].$this->createUrl(implode(Yii::app()->controller->module->recoveryUrl),array("activationKey" => $user->activationKey, "email" => $user->email));
